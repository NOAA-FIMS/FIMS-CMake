...
public:
...
#ifdef ATL_MODEL
      VonBertalanffyModel<atl::Variable<double> >* model_1 =
        VonBertalanffyModel<atl::Variable<double> >::getInstance();
  ...
  void finalize(Rcpp::NumericVector v){
    ...
    for(int i =0; i < model->predicted.size(); i++){
...
#ifdef ATL_MODEL
      VonBertalanffyModel<atl::Variable<double> >* model =
        VonBertalanffyModel<atl::Variable<double>>::getInstance();
      ...
      this->k.value = model->k;
      this->a_min.value = model->a_min.GetValue();
      this->l_inf.value = model->l_inf.GetValue();
      this->predicted = Rcpp::NumericVector(model->predicted.size());
      for(int i =0; i < model->predicted.size(); i++){
        this->predicted[i] = model->predicted[i].GetValue();
      }
#endif
  }
...
template<typename T>
class atl_objective_function: public atl::ObjectiveFunction<T>{
    
public:
    void Initialize(){
        VonBertalanffyModel<atl::Variable<T> >* model =
          VonBertalanffyModel<atl::Variable<T> >::getInstance();
        
        for(int i =0; i < model->parameters.size(); i++){
            this->RegisterParameter(*model->parameters[i]);
        }
    }
...
bool minimize(){
    atl_objective_function<double> obj;
    obj.Initialize();
    
    atl::LBFGS<double> min;
    min.SetObjectiveFunction(&obj);
...
#ifdef ATL_MODEL
    Rcpp::function("minimize", minimize);
#endif
};
